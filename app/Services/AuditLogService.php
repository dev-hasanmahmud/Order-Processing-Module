...
{
    public static function generateLogs(OrderApplication $orderApplication)
    {
        $changes      = [];
        $statuses     = Status::pluck('name', 'id');
        $users        = User::pluck('name', 'id');

        foreach ($orderApplication->logs as $log) {
                ...
                foreach ($differences as $key => $difference) {
                    ...
                    if ($key == 'status_id') {
                        ...
                        if (in_array($difference, [3, 4, 6, 7])) {
                            ...
                            $value['comment'] = $orderApplication->comments->where('user_id', $current[$column])->first()->comment_text;
                        }
